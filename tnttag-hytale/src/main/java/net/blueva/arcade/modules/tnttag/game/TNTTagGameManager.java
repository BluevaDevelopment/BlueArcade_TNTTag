...
public class TNTTagGameManager {
    ...
    private void detonateTaggedPlayers(GameContext<Player, Location, World, String, ItemStack, String, Holder, Entity> context,
        ...
        for (Player player : new ArrayList<>(context.getAlivePlayers())) {
            if (!currentTagged.contains(player.getUuid())) {
                continue;
            }
            ...
            runtime.recordElimination(player.getUuid());
        }
    ...
    private void selectTaggedPlayers(GameContext<Player, Location, World, String, ItemStack, String, Holder, Entity> context,
        ...
        for (int i = 0; i < taggedCount; i++) {
            ...
            runtime.getTaggedPlayers().add(player.getUuid());
            runtime.getTaggedThisRound().add(player.getUuid());
    ...
    public void finishGameIfPossible(GameContext<Player, Location, World, String, ItemStack, String, Holder, Entity> context) {
        ...
        if (alivePlayers.size() == 1) {
            ...
            if (runtime.setWinnerIfAbsent(winner.getUuid())) {
                context.setWinner(winner);
                statsService.recordWin(winner);
            }
        }

        registerEliminationPodium(context, runtime);
    ...
    public void passTNT(GameContext<Player, Location, World, String, ItemStack, String, Holder, Entity> context,
        ...
        if (!tagged.contains(from.getUuid())) {
            return;
        }
        ...
        if (tagged.contains(to.getUuid())) {
            return;
        }

        tagged.remove(from.getUuid());
        handleTaggedStatusChange(context, from, false);

        tagged.add(to.getUuid());
        runtime.getTaggedThisRound().add(to.getUuid());
    ...
    private List<String> getTaggedPlayerNames(GameContext<Player, Location, World, String, ItemStack, String, Holder, Entity> context) {
        ...
        for (Player player : context.getPlayers()) {
            if (tagged.contains(player.getUuid())) {
                String name = player.getPlayerRef() != null ? player.getPlayerRef().getUsername() : null;
    ...
    private void registerEliminationPodium(GameContext<Player, Location, World, String, ItemStack, String, Holder, Entity> context,
        ...
        List<UUID> eliminated = new ArrayList<>(runtime.getEliminationOrder());
        if (eliminated.isEmpty()) {
            return;
        }
        Collections.reverse(eliminated);
        UUID winnerId = runtime.getWinner();
        for (UUID playerId : eliminated) {
            Player player = findPlayerById(context, playerId);
            if (player == null) {
                continue;
            }
    ...
    private void playExplosionEffect(Player player) {
        ...
        if (location != null) {
            ...
            visualEffectsAPI.playDeathEffect(player);
        }
    }
